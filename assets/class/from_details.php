...
class select_from_details extends titels{
	...
	public function equpment_name(){		
		$sql = "SELECT * FROM pc_it_deails_tb  GROUP BY equpment_name ORDER BY equpment_name ASC";
...
public function department_name(){		
		      $sql = "SELECT * FROM pc_it_deails_tb  GROUP BY department_name ORDER BY department_name ASC";
	          $result = mysql_query($sql);
	          $num_rows = mysql_num_rows($result);
			  if ($num_rows > 0) {
...
public function owner_name(){		
		$sql = "SELECT * FROM pc_it_deails_tb  GROUP BY owner_name ORDER BY owner_name ASC";
	    $result = mysql_query($sql);
	    $num_rows = mysql_num_rows($result);
			 if ($num_rows > 0) {
...
public function os_name(){		
		$sql = "SELECT * FROM pc_it_deails_tb  GROUP BY os_name ORDER BY os_name ASC";
	    $result = mysql_query($sql);
	    $num_rows = mysql_num_rows($result);
			     if ($num_rows > 0) {
...
public function office_name(){		
		$sql = "SELECT * FROM pc_it_deails_tb  GROUP BY office_name ORDER BY office_name ASC";
	    $result = mysql_query($sql);
	    $num_rows = mysql_num_rows($result);
			      if ($num_rows > 0) {
...
public function model(){		
		$sql = "SELECT * FROM pc_it_deails_tb  GROUP BY  	model_name ORDER BY  	model_name ASC";
	    $result = mysql_query($sql);
	    $num_rows = mysql_num_rows($result);
			      if ($num_rows > 0) {
...
public function ram_capacity(){		
		 $sql = "SELECT * FROM pc_it_deails_tb  GROUP BY ram_capacity ORDER BY ram_capacity ASC";
	     $result = mysql_query($sql);
	     $num_rows = mysql_num_rows($result);
			    if ($num_rows > 0) {
...
public function barcord_check_dates(){		
		$sql = "SELECT * FROM pc_it_check_tb GROUP BY check_date like'%".date('Y-m')."%' ";
	    $result = mysql_query($sql);
	    $num_rows = mysql_num_rows($result);
			if ($num_rows  > 0) {
			echo '<select name="check_date" >';				
			if(!empty($this->check_date)){echo '<option value="'.$this->check_date.'">'.$this->check_date.'</option>';}
			else{ echo '<option  value="">'.$this->plz_select_one.'</option>';}		
			echo '<option value="">-</option>';		 
			while($row=mysql_fetch_array($result)) {
				$db_dat=explode("-",$row['check_date']);
				if(!empty($row['check_date'])){
			echo '<option>'.$db_dat[0].'-'.$db_dat[1].'</option>';
					 }
			}
			           }
			else {				
